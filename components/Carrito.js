...
export default function Carrito() {
    ...
    return (
        <div style={{ display: cart.isOpen ? 'block' : 'none' }} className="z-50 fixed right-0 top-0 bg-white w-screen md:w-3/6 xl:w-4/12 h-screen p-6 shadow-2xl shadow-slate-950 flex flex-col">
            ...
            {cart.items.length === 0 ? <div className=" h-full flex items-center justify-center flex-col"><Image className=" w-56" src={Sad}></Image><div className="text-3xl font-bold text-slate-900">Carrito vacio</div></div> : <div>
                <div className="text-2xl font-bold text-slate-900">Tus productos</div>
                <div>
                    {cart.items.map((item) => (
                        <div>
                ...
                <div>
                    <Link href='#' className=" w-auto flex justify-center cursor-pointer bg-green-600 hover:bg-green-900 text-white px-6 py-2 rounded-full font-sans">Ir a pagar</Link>
                </div>
            </div>}
        </div>
    )

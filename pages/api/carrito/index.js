...
export default async function handler(req,res){
    ...
    console.log('---------------\nDetalle Boleta')
    await pool.query('INSERT INTO PAGO SET ?',{buyOrder:buyO,precio:amt,cantidad:sum,fecha:date,estado:'pendiente',usu:user})
    for (let index = 0; index < lis.length; index++) {
        const element = lis[index]
        console.log(`N(${index}): ${element.id} ${element.title} ${element.qty} ${element.price}`)
